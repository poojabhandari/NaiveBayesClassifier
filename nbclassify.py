...
def writeClass(path,fw):
    
    for root, dirs, files in os.walk(path):
        for f1 in files:
            if f1.endswith(".txt") and f1 != 'README.txt':
                filee = os.path.join(root, f1)
                with open(filee, 'r') as my_file:                
                    fileContent = my_file.read()
                    scoreHash = {}
                    scoreHash['negDec'] = math.log10(0.25)
                    scoreHash['negTru'] = math.log10(0.25)
                    scoreHash['posDec'] = math.log10(0.25)
                    scoreHash['posTru'] = math.log10(0.25)
                    for line in fileContent.splitlines():
                        ...
                        for word in wordArray:                            
                            ...
                            if negDecprobTable.has_key(wordStr):                                                                  
                                scoreHash['negDec'] = scoreHash['negDec'] + negDecprobTable[wordStr]                                                            
                            if negTruprobTable.has_key(wordStr):                                
                                scoreHash['negTru'] = scoreHash['negTru'] + negTruprobTable[wordStr]                                
               
                            if posDecprobTable.has_key(wordStr):                                                            
                                scoreHash['posDec'] = scoreHash['posDec'] + posDecprobTable[wordStr]                                                                
                            if posTruprobTable.has_key(wordStr):                                
                                scoreHash['posTru'] = scoreHash['posTru'] + posTruprobTable[wordStr]
                                
                    classofReview = max(scoreHash, key=scoreHash.get)
                    if classofReview == 'negDec':
                        fw.write('deceptive' + ' ' + 'negative' + ' ' + f1+' \n')                    
                    elif classofReview == 'negTru':
                        fw.write('truthful' + ' ' + 'negative' + ' ' + f1 + '\n')
                    elif classofReview == 'posDec':
                        fw.write('deceptive' + ' ' + 'positive' + ' ' + f1 + '\n')
                    elif classofReview == 'posTru':
                        fw.write('truthful' + ' ' + 'positive' + ' ' + f1 + '\n')
                        
                                 
with open('nbmodel.txt', 'r') as fp:
    jsonData = json.load(fp)
    
for mainCategory in jsonData.keys():
    if mainCategory == 'negDec':
        negDecprobTable = jsonData[mainCategory]
    elif mainCategory == 'negTru':
        negTruprobTable = jsonData[mainCategory]
    elif mainCategory == 'posDec':
        posDecprobTable = jsonData[mainCategory]
    elif mainCategory == 'posTru':
        posTruprobTable = jsonData[mainCategory]


testdataPath = sys.argv[1]
fw1 = open('nboutput.txt','w')
writeClass(testdataPath,fw1)
